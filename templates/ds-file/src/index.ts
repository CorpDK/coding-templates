...
const PORT = Number(process.env.DS_FILE_PORT);
if (!PORT) throw new Error("DS_FILE_PORT env var is required");
...
server.listen(PORT, () => {
  console.log(`@corpdk/ds-file  HTTP  http://localhost:${PORT}/graphql`);
  console.log(`@corpdk/ds-file  WS    ws://localhost:${PORT}/graphql`);
});
