...
const PORT = Number(process.env.DS_DDB_PORT);
if (!PORT) throw new Error("DS_DDB_PORT env var is required");
...
server.listen(PORT, () => {
  console.log(`@corpdk/ds-ddb  HTTP  http://localhost:${PORT}/graphql`);
  console.log(`@corpdk/ds-ddb  WS    ws://localhost:${PORT}/graphql`);
});
