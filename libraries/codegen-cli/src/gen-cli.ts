...
const RUNTIME_LINES = [
  ...
  "  }",
  "",
  "  let h = \"Usage: \" + name + \" <command> [options]\\n\\n\";",
  "",
  "  if (queries.length) {",
  "    h += \"Queries:\\n\";",
  ...
  "",
  "  if (subs.length) {",
  "    h += \"Subscriptions (streams NDJSON until Ctrl-C or --count/--timeout):\\n\";",
  "    for (const [cmd, op] of subs) {",
  "      h += \"  \" + cmd + \" [--count N] [--timeout Ms]\\n\";",
  "      h += \"    \" + op.description + \"\\n\";",
  "    }",
  "    h += \"\\n\";",
  "  }",
  "",
  "  h += \"Input (mutations/queries with variables):\\n\";",
  "  h += \"  --input <file.json>  Read variables from JSON file\\n\";",
  "  h += \"  (stdin)              Pipe JSON when stdin is not a TTY\\n\";",
  "  h += \"  Merge order: --input file < piped stdin < explicit flags\\n\\n\";",
  "",
  "  h += \"Output: newline-delimited JSON on stdout. Errors on stderr (exit 1).\\n\\n\";",
  "",
  "  h += \"Environment:\\n\";",
  "  h += \"  DS_HTTP_URL  \" + DS_HTTP_URL + \"\\n\";",
  "  h += \"  DS_WS_URL    \" + DS_WS_URL + \"\\n\";",
  "",
  "  process.stdout.write(h);",
  "}",
  "",
  "function coerceArg(value, graphqlType) {",
  "  const base = graphqlType.replace(/[!\\[\\]]/g, \"\");",
  ...
  "  if (json.errors) {",
  "    process.stderr.write(JSON.stringify(json.errors, null, 2) + \"\\n\");",
  "    process.exit(1);",
  "  }",
  "  return json;",
  "}",
  ...
  "      catch (err) { process.stderr.write(\"Invalid JSON on stdin: \" + err.message + \"\\n\"); process.exit(1); }",
  "    }",
  "  }",
  "  return vars;",
  "}",
  ...
  "      process.stderr.write(\"Error: --\" + n + \" is required\\n\");",
  "      process.exit(1);",
  "    }",
  "  }",
  "",
...
export function generateCli(ops: OperationDescriptor[], config: Config): string {
