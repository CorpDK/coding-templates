...
function drizzleImportLine(needsInArray: boolean, needsBulkFilterLock: boolean): string {
  const parts = ["eq"];
  if (needsInArray || needsBulkFilterLock) parts.push("inArray");
...
export function generateRepository(
  ...
  const needsBulkFilterLock = full;
...
${drizzleImportLine(needsInArray, needsBulkFilterLock)}
...
${updateSet}
  ...
  async delete(id: string, ctx: RepositoryContext, conn: typeof db = db) {
    try {
      ...
      ${soft ? `const actor = resolveActorId(ctx.actorId);
      await conn.update(table).set({ deletedAt: new Date(), deletedBy: actor }).where(eq(table.id, id));` : `await conn.delete(table).where(eq(table.id, id));`}
